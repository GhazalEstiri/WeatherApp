    ...
    <div className="p-5 flex flex-col gap-6">
      <header className="relative h-130 overflow-hidden rounded-xl">
        ...
        <div className="relative z-10 flex flex-col items-center pt-20 gap-10 mt-30">
          <h1 className="text-white font-bold text-6xl">Weather Cities</h1>

          <div className="flex items-center gap-0 md:gap-1 lg:gap-4 px-20 md:px-8 bg-[#101C2E] h-16 rounded-[50px]  md:w-[50%] justify-center w-full">
            <span className="text-3xl text-white md:ml-7">⌕</span>

            <input
              type="text"
              value={search}
              className="flex-1 bg-transparent  text-sm md:text-xl text-white placeholder:text-[#7F8EAF] outline-none"
            ...
            <button
              ...
              className="text-white bg-blue-600 md:px-6 md:py-3 md:mr-6 lg:-mr-5 px-3 py-2 rounded-full"
            >
              Search
            </button>
          </div>
      ...
      <div className=" grid sm:grid-cols-3 gap-7 mx-auto grid-cols-1">
        {cities.map((city) => {
          return (
            <Link
              key={city.name}
